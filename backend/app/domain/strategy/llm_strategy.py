...
from app.core.config import APP_DIR, CONFIG_DIR
from app.models import ParseResult, ParsedOp
...
def _load_dotenv():
    ...
    # 基于配置中心 APP_DIR/CONFIG_DIR 定位（本模块 2026-09-21 从 backend/app/ 下沉到
    # app/domain/strategy/ 后，dirname(__file__) 基准变了，会静默读不到 .env）
    for cand in (
        os.path.join(CONFIG_DIR, ".env"),           # backend/config/.env
        os.path.join(APP_DIR, ".env"),              # backend/.env
        os.path.join(os.path.dirname(APP_DIR), ".env"),  # 仓库根 .env
    ):
...
def _llm_cfg():
    ...
    """
    from app.settings.llm_settings import get_llm_cfg
    cfg = get_llm_cfg()
    return (
        cfg["base_url"].rstrip("/"),
        cfg["api_key"],
