...
- 写：apply / delete / restart 走 HTTP POST（Bearer token 认证），agent 本地执行 kubectl。
...
_DEVICES_PATH = config_path("box_devices.json")
...
# ------------------------- agent 连接地址：候选回退 -------------------------
# 地址一律用「可移植的公网/域名」写进配置（box_devices.json cloud.host），局域网/本地地址
# 不落配置，由运行环境自己推导（见 core/netinfo.gateway_host）。候选顺序：
#   [记忆的生效地址 → cloud.host（配置） → broker.host（配置，与 agent 同机） → 宿主网关]
# 逐个尝试：成功者记忆 _ACTIVE_HOST_TTL 秒（后续请求直接命中，不必再吃一次失败连接），
# 失败者进冷却 _FAIL_COOLDOWN 秒（避免每个请求都在不可达地址上重复等超时）。
# 宿主网关候选让「平台容器 + 同机云端」在公网回环（NAT hairpin）未放行时仍能用。
_ACTIVE_HOST_TTL = 300.0
_FAIL_COOLDOWN = 60.0
_active_host: Dict[str, Any] = {"host": "", "ts": 0.0}
_fail_until: Dict[str, float] = {}


def _broker_host() -> str:
    """云端 Broker 地址（box_config.json broker.host）：与 agent 同机，可作回退候选。"""
    try:
        with open(_BROKER_CFG_PATH, "r", encoding="utf-8") as f:
            bcfg = json.load(f) or {}
        return str((bcfg.get("broker") or {}).get("host") or "").strip()
    except Exception:  # noqa: BLE001
        return ""


def candidate_hosts() -> List[str]:
    """agent HTTP 候选地址（按优先级去重）：记忆的生效地址 → cloud.host → broker.host → 宿主网关。"""
    cfg = agent_cfg()
    now = time.time()
    with _LOCK:
        active = str(_active_host.get("host") or "")
        ts = float(_active_host.get("ts") or 0)
    hosts: List[str] = []
    if active and now - ts < _ACTIVE_HOST_TTL:
        hosts.append(active)
    for h in (cfg["host"], _broker_host(), gateway_host()):
        h = str(h or "").strip()
...
def active_host() -> str:
    """当前记忆的生效 agent 地址（空串=尚未成功连过任何候选）。"""
    with _LOCK:
        return str(_active_host.get("host") or "")
...
def _http(path: str, method: str = "GET", body: "Optional[Dict[str, Any]]" = None,
    ...
    地址按 candidate_hosts() 依次尝试（见上方说明），任一成功即记忆该地址；
    全部失败时返回汇总了各候选失败原因的不可达错误。
    """
    cfg = agent_cfg()
    port, token = cfg["agent_port"], cfg["agent_token"]
    if not token:
        return False, None, "云端 agent 未配置：请在「总览 → 配置」填写 agent_token（并确认 agent 端口）"
    hosts = candidate_hosts()
    data = json.dumps(body).encode("utf-8") if body is not None else None
    with _LOCK:
        cooled = {h for h, until in _fail_until.items() if until > time.time()}
    # 全部候选都在冷却（说明当前都不通）时仍探测首选，避免错误信息永远滞后于现场恢复
    plan = [h for h in hosts if h not in cooled] or hosts[:1]
    errors: List[str] = []
    for host in plan:
        ...
        try:
            ...
            with _LOCK:
                _fail_until[host] = time.time() + _FAIL_COOLDOWN
                if str(_active_host.get("host") or "") == host:
                    _active_host["host"], _active_host["ts"] = "", 0.0
            continue
        with _LOCK:
            _active_host["host"], _active_host["ts"] = host, time.time()
            _fail_until.pop(host, None)
        return True, payload, ""
    if not errors:
        return False, None, "云端 agent 不可达（无可用候选地址）"
    return False, None, ("云端 agent 不可达（" + "；".join(errors)
                         + "）：请确认云端 agent 已启动（install_agent.sh）且网络可达")
...
def health() -> Dict[str, Any]:
    ...
    out: Dict[str, Any] = {
        ...
        # 实际命中的候选地址与全部候选（诊断「本地/生产地址不同」问题：cloud.host=172.18.0.1
        # 在本机不可达时会自动回退到 broker.host）
