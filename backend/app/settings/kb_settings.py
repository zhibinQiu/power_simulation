...
from app.core.config import APP_DIR, config_path
from app.core.json_cfg import JsonCfgStore

CONFIG_PATH = config_path("kb_cfg.json")
ENV_MAP = {"root_path": "KB_ROOT"}
# 默认知识库根目录：backend/knowledge（绝对路径，不依赖进程 CWD；
# 与知识库服务文档及历史数据位置一致，避免相对路径导致数据分散）
DEFAULT_KB_PATH = os.path.join(APP_DIR, "knowledge")
