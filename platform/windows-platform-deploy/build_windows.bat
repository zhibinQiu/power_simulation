...
REM  位置：platform\windows-platform-deploy\
...
REM     steelcarbon_twin.iss（安装包脚本）、icon.ico（图标）、
...
python -m PyInstaller --noconfirm --clean --onedir --windowed ^
  ...
  --icon "%CD%\platform\windows-platform-deploy\icon.ico" ^
  ...
  --paths backend ^
  --specpath platform\windows-platform-deploy ^
  platform\windows-platform-deploy\desktop_launcher.py
if errorlevel 1 goto :fail
