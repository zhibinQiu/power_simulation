...
#  位置：platform/mac-deploy/
#  用法：
#    ./platform/mac-deploy/build_mac.sh             # 打包为 SteelCarbonTwin.app（原生客户端窗口）
#    ./platform/mac-deploy/build_mac.sh console     # 打包为终端运行的 onedir 版本（可见日志，Ctrl+C 退出）
#    ./platform/mac-deploy/build_mac.sh universal   # 额外构建 universal2（兼容 Intel + Apple Silicon）
#
#  产物（位于项目根 dist/）：
...
"${RUNPY[@]}" platform/mac-deploy/build_icons.py
...
"${RUNPY[@]}" -m PyInstaller --noconfirm --clean --onedir \
  ...
  --icon "$(pwd)/platform/mac-deploy/icon.icns" \
  --collect-all uvicorn --collect-all websockets --collect-all webview \
  --add-data "$(pwd)/frontend/dist:frontend/dist" \
  --paths backend \
  --specpath platform/mac-deploy \
  "${EXTRA_ARGS[@]}" \
  platform/mac-deploy/desktop_launcher.py
