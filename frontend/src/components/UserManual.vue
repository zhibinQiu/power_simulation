...
| 工具条 | 菜单条下方 | 「主页 / 视图 / 数据 / 编排」四个标签页，按任务切换按钮组 |
...
| 环境（子菜单） | 切换 3D 环境主题：森林 / 城市 / 沙漠 / 海岸，勾选项为当前环境 |
| 工具栏 · 显示「操作」工具 | 开关切换，控制工具条是否显示「撤销 / 重做」等操作按钮 |
| 重置相机视角 (R) | 将相机重置为园区俯瞰视角 |

### 编辑

| 菜单项 | 操作逻辑 |
| --- | --- |
| 进入流程编排 / 完成编排 | 切换「3D 场景」与「编排画布」两种工作模式（见第九章） |
...
> 操作逻辑要点：菜单项执行后大多在命令行窗口输出一条系统反馈，用于确认操作已生效。`,
  },
  {
    id: 'ribbon',
    title: '工具条操作',
    body: `## 三、工具条（主页 / 视图 / 数据 / 编排）

工具条与菜单条配合使用，按任务分组。点击标签即切换按钮组；进入「编排」标签时会自动展开右侧检视器。

### 主页

- **运行 / 退出仿真**：主开关。未进入仿真时显示「运行」，点击后进入仿真模式（场景右上角出现对比浮层，所有修改仅预览）；已进入时显示「退出仿真」，点击后恢复仿真前状态。
- **撤销 / 重做**：默认隐藏，需在「视图 → 工具栏 · 显示「操作」工具」中开启。

### 视图

| 按钮 | 操作逻辑 |
| --- | --- |
| 相机预设（俯视/正视/侧视/聚焦/全景） | 需先在场景或资产树选中工序，再点击对应视角按钮聚焦 |
| 重置视角 | 相机回到园区俯瞰（同快捷键 R） |
...
- **导出报告**：与「文件 → 导出分析报告」等价，打开右侧报告面板。
- **策略生成**：输入自然语言描述优化目标（如「焦比降低 5%」），由智能体解析为策略（规划中）。

### 编排

| 按钮 | 操作逻辑 |
| --- | --- |
| 进入编排 / 完成编排 | 切换至编排画布工作模式 / 完成编排回到 3D 场景 |
| 放大 / 缩小 / 适配视图 / 自动布局 | 编排画布的视图控制 |
| 撤销 / 重做 | 编排历史操作回退 |
| 长流程示例 / 短流程示例 / 清空 | 一键载入示例流程 / 清空画布全部节点连线 |
...
- 配合工具条「相机预设」按钮或命令行 \`view top|front|side|focus\` 切换视角。
...
- **进入**：点击「运行」、输入 \`/sim\` 或执行任一「策略仿真」。
- **预览**：进入后所有参数修改仅作用于预览，不改动原始方案。
- **退出**：点击「退出仿真」或输入 \`stop\`，自动恢复进入前的全部状态。
- **自然语言**：仿真模式下可直接输入指令（如「焦比降低 10%」），由智能体解析为操作（规划中）。`,
  },
  {
    id: 'sim',
    title: '仿真运行与情景',
...
- 入口：菜单「编辑 → 进入流程编排」、工具条「编排 → 进入编排」或命令行 \`edit\`。
- 退出：菜单「编辑 → 完成编排」、工具条「编排 → 完成编排」或命令行 \`done\`。
...
1. 菜单「文件 → 导出分析报告」或工具条「数据 → 导出报告」。
2. 右侧打开「报告面板」，配置：
   - 报告标题；
   - 生成引擎（AI 生成 / 本地模板）；
   - 分析深度。
3. 点击 **「生成报告」**，等待生成完成。
4. 生成后在面板内查看、复制或打开 HTML 报告文件。

### 报告内容

报告基于当前仿真结果自动汇总：全厂能耗与碳排放总量、各工序排放分布、所应用策略的减排效果对比、优化建议。`,
  },
  {
    id: 'faq',
    title: '快捷键与常见问题',
...
**Q8：环境切换后 3D 场景没变化？**
环境主题（森林/城市/沙漠/海岸）只影响场景背景与氛围，不影响设施与仿真数据；若画面异常可重置相机视角。`,
  },
]
