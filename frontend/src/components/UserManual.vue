...
| 项 | 说明 |
| --- | --- |
| 实时链路 | 数据源运行状态（模拟 / WebSocket / HTTP），点击可打开「连接」面板 |
...
- 列出 **模拟 / WebSocket / HTTP 轮询** 各链路；当前启用的链路显示绿色「在线」，未启用显示灰色「离线」；
- 每条链路提供**连接 / 断开**操作（模拟源恒在线）；
...
| 类型 | 说明 | 配置项 |
| --- | --- | --- |
| 模拟 | 内置随机数发生器按平台参数生成读数，默认启用，无需配置 | 刷新间隔（默认 1 秒） |
...
先点击面板内「数据源配置」按钮，或在文件菜单「连接数据源…」中新建一个 WebSocket / HTTP 数据源并启用；默认「模拟」数据源始终在线，用于保证界面有数据。
