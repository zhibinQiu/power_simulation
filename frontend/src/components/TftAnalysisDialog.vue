  ...
  <Teleport to="body">
    <div class="tad-mask" @mousedown.self="onClose">
      <div class="tad-dialog" ref="dialogEl" :style="dialogStyle" :class="{ dragging }">
        <!-- 标题栏（按住可拖动弹窗）：平台弹窗标题栏风格 -->
        <div class="tad-titlebar" @mousedown.prevent="onTitleDown">
          <span class="tad-icon">◧</span>
          <span class="tad-title">{{ t('高炉数值仿真分析') }}</span>
          <span class="tad-subtitle">blast_furnace · {{ t('TFT 热风温度仿真') }}</span>
          <span class="tad-spacer"></span>
          <button class="tad-restore" :disabled="!restoreDirty" :title="t('恢复打开弹窗时的高炉参数')" @click.stop="onRestore">{{ t('恢复仿真前') }}</button>
          <button class="tad-close" :title="t('关闭 (Esc)')" @click="onClose">
        ...
        <div class="tad-body">
          ...
          <div class="tad-left">
            ...
            <div class="tad-grid">
              ...
              >
                <div class="cell-head">
                  <span class="cell-name">{{ t(a.label) }}</span>
                  <span class="cell-val mono">{{ fmt(a.curVal) }} {{ a.unit }}<sup v-if="a.coalOxyInc > 0.5" class="cell-eff">{{ t('富氧') }}+{{ fmt(a.coalOxyInc) }}</sup></span>
                </div>
                ...
                <div v-if="tip && tip.key === a.key" class="tad-tip" :style="{ left: tip.px + 'px', top: tip.py + 'px' }">
                  <div class="t1">{{ t(a.label) }} = {{ fmt(tip.pt.x) }} {{ a.unit }}</div>
            ...
            <div class="tad-setter">
              <div class="ts-head">
                <span class="ts-lbl">{{ t(ax.label) }}</span>
                <span class="ts-val mono">
                  <template v-if="ax.key === 'coal_inj'">
                    <b>{{ fmt(effFuel.coal_inj) }}</b> {{ ax.unit }}
                    <span style="font-size:10px;color:var(--faint);margin-left:4px;">({{ t('设定') }} {{ fmt(curX) }})</span>
                  </template>
                  <template v-else>
                ...
                <span class="ts-badge" :class="applied ? 'ok' : 'dirty'">
                  <i class="bd-dot"></i>{{ applied ? t('已应用') : t('待应用') }}
                </span>
              </div>
              ...
              <div v-if="ax.key === 'coal_inj'" class="ts-eff">
                <template v-if="(effFuel.coal_inj || 0) - (Number.isFinite(curX.value) ? curX.value : 0) > 0.5">
                  {{ t('喷煤比可直接拖动调节（设定煤比）。设定煤比 {a} kg/t + 富氧增量 {b} = 有效煤比', { a: fmt(curX), b: fmt((effFuel.coal_inj || 0) - (Number.isFinite(curX.value) ? curX.value : 0)) }) }} <b>{{ fmt(effFuel.coal_inj) }} kg/t</b>。{{ t('曲线/悬停/当前点均按有效煤比展示，X 轴固定 0~220。') }}
                </template>
                <template v-else>
                  {{ t('喷煤比可直接拖动调节（设定煤比）。设定煤比 = 有效煤比 {a} kg/t（无富氧增量）。X 轴固定 0~220。', { a: fmt(effFuel.coal_inj) }) }}
                </template>
              </div>
              <div v-else-if="ax.key === 'coke_rate'" class="ts-eff">
                {{ t('焦比可直接拖动设定（设定焦比）。设定焦比 {a} kg/t 时，煤比保持冻结 =', { a: fmt(curX) }) }} <b>{{ fmt(effFuel.coal_inj) }} kg/t</b>（{{ t('含富氧派生') }} +{{ fmt(effFuel.coal_oxy_inc) }}），{{ t('仅焦比变动、不反向推导煤比。TFT 与 CO₂ 随焦比直接联动。') }}
              </div>
              <input
                class="ts-range"
                type="range"
          ...
          <!-- 右栏：当前工况 + 灵敏度总览 + 策略建议 -->
          <div class="tad-right">
            <div class="tad-sec-title">{{ t('当前工况') }}</div>
            <div class="tad-cond">
              <div class="cond-main">
                <span class="cond-tft mono">{{ cur.tft.toFixed(0) }}</span>
                <span class="cond-unit">℃</span>
                <span class="cond-st" :style="{ color: cur.status.color }">
                  <i class="st-dot" :style="{ background: cur.status.color }"></i>{{ t(cur.status.label) }}
                </span>
              </div>
              <div class="cond-grid">
                <!-- 焦比/煤比：与主流程排放链路共用同一推算真源（utils/bfFuel），随操作参数联动 -->
                <div class="cg-item"><span class="cg-k">{{ t('焦比') }}<sup>{{ t('联动') }}</sup></span><b class="mono">{{ fmt(effFuel.coke_rate) }} kg/t</b></div>
                <div class="cg-item"><span class="cg-k">{{ t('喷煤比') }}<sup v-if="effFuel.coal_oxy_inc > 0.5">{{ t('富氧') }}+{{ fmt(effFuel.coal_oxy_inc) }}</sup></span><b class="mono">{{ fmt(effFuel.coal_inj) }} kg/t</b></div>
                <div class="cg-item"><span class="cg-k">{{ t('燃料比') }}<sup>{{ t('焦+煤') }}</sup></span><b class="mono">{{ fmt((Number(effFuel.coke_rate)||0) + (Number(effFuel.coal_inj)||0)) }} kg/t</b></div>
                <div class="cg-item"><span class="cg-k">{{ t('热风温度') }}</span><b class="mono">{{ fmt(effFuel.hot_blast_temp) }} ℃</b></div>
                <div class="cg-item"><span class="cg-k">{{ t('富氧率') }}</span><b class="mono">{{ fmt(effFuel.oxygen_enrich) }} %</b></div>
              </div>
              <div class="cond-tip" style="margin-top:6px">
                {{ t('燃料比 = 焦比 + 煤比。富氧↑ → 煤比 +15 kg/t·1%，焦比按煤粉成分算出的置换比 RR={rr} 反向联动。因 RR<1，燃料比随富氧微升为正常现象；真实收益是', { rr: (replacementRatio||0).toFixed(2) }) }}<b>{{ t('以煤代焦') }}</b>{{ t('（降成本）+ 提产。') }}
              </div>
              <!-- CO2 排放：随配料比（焦比/喷煤比）联动同步展示（t/h 口径 = 强度 × 铁水产量） -->
              <div class="cond-co2">
                <div class="co2-main">
                  <span class="co2-k">{{ t('CO₂ 排放') }}</span>
                  ...
                  <span class="cond-st" :style="{ color: co2.level.color }">
                    <i class="st-dot" :style="{ background: co2.level.color }"></i>{{ t(co2.level.label) }}
                  </span>
                </div>
                <div class="co2-grid">
                  <div class="cg-item"><span class="cg-k">{{ t('铁水产量') }}</span><b class="mono">{{ fmt(co2.hot_metal) }} t/h</b></div>
                  <div class="cg-item"><span class="cg-k">{{ t('入炉碳 C_in') }}</span><b class="mono">{{ fmt(co2.C_in) }} kg C/t</b></div>
                  <div class="cg-item"><span class="cg-k">{{ t('产品扣碳 C_HM/C_slag') }}</span><b class="mono">{{ fmt(co2.C_HM) }}/{{ fmt(co2.C_slag) }} kg C/t</b></div>
                  <div class="cg-item"><span class="cg-k">{{ t('熔剂分解 CO₂') }}</span><b class="mono">+{{ fmt(co2.CO2_flux) }} kg/t</b></div>
                  <div class="cg-item"><span class="cg-k">{{ t('排放碳 C_emit') }}</span><b class="mono">{{ fmt(co2.C_emit) }} kg C/t</b></div>
                  <div class="cg-item"><span class="cg-k">{{ t('风口 / 非风口') }}</span><b class="mono">{{ fmt(co2.CO2_rate_raceway) }} / {{ fmt(co2.CO2_rate_other) }} t/h</b></div>
                </div>
                <div class="co2-tip">{{ (co2.level && co2.level.desc) || t('国标口径：CO₂ = (入炉碳AD×NCV×CC − 铁水溶碳 − 渣带碳) × 44.009/12.011 + 熔剂分解；炉尘碳计入排放。') }}</div>
              </div>
              
              <div class="cond-tip">{{ t('可调参数：风温 / 富氧率 / 风量 / 喷煤比 / 焦比。风温与富氧为升温主力；喷煤以氢代碳可减碳但压低 TFT，需富氧/风温补偿。煤比偏离基准 130 kg/t 时焦比按 Δ焦比=−RR×Δ煤比 联动（RR≈0.89）；富氧每 +1% 允许多喷 15 kg/t 煤粉再联动降焦比。') }}<b>{{ t('焦比为独立设定') }}</b>{{ t('：拖动只改焦比本身，不反推煤比。') }}</div>
            </div>

            <div class="tad-sec-title">{{ t('相对基准变化') }} <span class="sec-sub">{{ t('基准＝打开弹窗时工况') }}</span></div>
            <div class="tad-delta" v-if="delta">
              <div class="d-row">
                <span class="d-k">{{ t('焦比') }}</span>
              ...
              <div class="d-foot">{{ t('Δ 与 Δ% 均按每吨铁水折算：焦比/喷煤比 kg/tHM，CO₂ kg CO₂/tHM（不受铁水产量影响）。基准固定为打开弹窗时的工况，当前随滑块实时预览。') }}</div>
            </div>

<!-- 炉渣碱度 R₂：随炉料配比/燃料比（含富氧置换联动）/物料详细化学成分实时联动，滑块预览同步刷新 -->
            <div class="cond-slag" v-if="slagInfo">
              <div class="co2-main">
                <span class="co2-k">{{ t('炉渣碱度 R₂（CaO/SiO₂）') }}</span>
                <span class="slag-val mono">{{ slagInfo.r2.toFixed(2) }}</span>
                <span class="cond-st" :style="{ color: slagColor }">
                  <i class="st-dot" :style="{ background: slagColor }"></i>{{ t(slagInfo.level.txt) }}
                </span>
              </div>
              <div class="co2-main">
                <span class="co2-k">{{ t('三元碱度 R₃（(CaO+MgO)/SiO₂）') }}</span>
                <span class="slag-val mono">{{ slagInfo.r3.toFixed(2) }}</span>
                <span class="cond-st" :style="{ color: slagColor }">
                  <i class="st-dot" :style="{ background: slagColor }"></i>{{ t(slagInfo.r3Level.txt) }}
                </span>
              </div>
              <div class="slag-grid">
                <div class="cg-item"><span class="cg-k">{{ t('有效燃料量') }}</span><b class="mono">{{ t('焦') }} {{ slagInfo.coke.toFixed(0) }} + {{ t('煤') }} {{ slagInfo.coal.toFixed(0) }} kg/t</b></div>
                <div class="cg-item"><span class="cg-k">{{ t('入渣 CaO') }}</span><b class="mono">{{ slagInfo.caoTotal.toFixed(1) }} kg/t</b></div>
                <div class="cg-item"><span class="cg-k">{{ t('入渣 SiO₂') }}</span><b class="mono">{{ slagInfo.sio2Total.toFixed(1) }} kg/t</b></div>
                <div class="cg-item"><span class="cg-k">{{ t('入渣 MgO') }}</span><b class="mono">{{ slagInfo.mgoTotal.toFixed(1) }} kg/t</b></div>
                <div class="cg-item"><span class="cg-k">{{ t('入渣 Al₂O₃') }}</span><b class="mono">{{ slagInfo.al2o3Total.toFixed(1) }} kg/t</b></div>
                <div class="cg-item"><span class="cg-k">{{ t('渣量估算') }}</span><b class="mono">{{ slagInfo.slagEst.toFixed(0) }} kg/t</b></div>
                <div class="cg-item cg-wide"><span class="cg-k">{{ t('炉渣氧化物成分（%）') }}</span><b class="mono">CaO {{ slagInfo.comp.cao.toFixed(1) }} · SiO₂ {{ slagInfo.comp.sio2.toFixed(1) }} · MgO {{ slagInfo.comp.mgo.toFixed(1) }} · Al₂O₃ {{ slagInfo.comp.al2o3.toFixed(1) }}</b></div>
              </div>
              <div class="slag-tbl">
                ...
                <div class="slag-tr" v-for="pt in slagInfo.parts" :key="pt.name">
                  <span>{{ t(pt.name) }}</span><span>{{ pt.rate.toFixed(0) }}</span>
                  <span>{{ pt.cao.toFixed(1) }}</span><span>{{ pt.sio2.toFixed(1) }}</span>
                  <span>{{ pt.mgo.toFixed(1) }}</span><span>{{ pt.al2o3.toFixed(1) }}</span>
                </div>
                <div class="slag-tr slag-sum"><span>{{ t('合计') }}</span><span></span><span>{{ slagInfo.caoTotal.toFixed(1) }}</span><span>{{ slagInfo.sio2Gross.toFixed(1) }}</span><span>{{ slagInfo.mgoTotal.toFixed(1) }}</span><span>{{ slagInfo.al2o3Total.toFixed(1) }}</span></div>
                <div class="slag-tr"><span>{{ t('Si 还原入铁扣减（[Si] 0.5%）') }}</span><span></span><span></span><span class="slag-neg">−{{ slagInfo.siDeduct.toFixed(1) }}</span><span></span><span></span></div>
                <div class="slag-tr slag-sum"><span>{{ t('入渣合计') }}</span><span></span><span>{{ slagInfo.caoTotal.toFixed(1) }}</span><span>{{ slagInfo.sio2Total.toFixed(1) }}</span><span>{{ slagInfo.mgoTotal.toFixed(1) }}</span><span>{{ slagInfo.al2o3Total.toFixed(1) }}</span></div>
              </div>
              <div class="co2-tip">{{ t('R₂ = ΣCaO/ΣSiO₂ = {cao} / {sio2}；R₃ = (CaO+MgO)/SiO₂ = ({cao}+{mgo}) / {sio2}（kg/tFe）。来源：烧结/球团/块矿脉石 + 焦炭/煤粉灰分（物料「详细化学成分 → 灰分组成」）+ 熔剂(石灰石)；MgO 主要来自熔剂与灰分、Al₂O₃ 主要来自矿脉石与煤灰；随滑块与物料成分实时联动。', { cao: slagInfo.caoTotal.toFixed(1), sio2: slagInfo.sio2Total.toFixed(1), mgo: slagInfo.mgoTotal.toFixed(1) }) }}</div>
            </div>



            <div class="tad-sec-title">{{ t('灵敏度总览') }}</div>
            <table class="tad-table">
              <thead>
                <tr><th>{{ t('操作参数') }}</th><th>{{ t('当前值') }}</th><th>ΔTFT</th><th>ΔCO₂</th><th>{{ t('趋势') }}</th></tr>
              </thead>
              <tbody>
                <tr v-for="a in axes" :key="a.key" :class="{ on: a.key === ax.key }" @click="axKey = a.key">
                  <td class="c-name">{{ t(a.label) }}</td>
                  ...
                  <td>
                    <span class="trend" :class="(axisStats[a.key] || {}).trendCls">
                      <i class="td-dot"></i>{{ t((axisStats[a.key] || {}).trend) }}
                    </span>
                  </td>
                </tr>
              </tbody>
            </table>
            <div class="tad-ax-desc">{{ t(ax.hint) }}</div>

            <div class="tad-sec-title">{{ t('策略建议') }}</div>
        ...
        <!-- 平台底部状态栏 -->
        <div class="tad-statusbar">
          <span class="sb-left">◧ {{ t('高炉数值仿真') }}</span>
          <span class="sb-item" :style="{ color: cur.status.color }">
            <i class="sb-dot" :style="{ background: cur.status.color }"></i>TFT {{ cur.tft.toFixed(0) }} ℃ · {{ t(cur.status.label) }}
          </span>
          <span class="sb-sep"></span>
          <span class="sb-item" :style="{ color: co2.level.color }">
            <i class="sb-dot" :style="{ background: co2.level.color }"></i>CO₂ {{ fmt(co2.CO2_rate) }} t/h
          </span>
          <span class="sb-sep"></span>
          <span class="sb-item">{{ t('炉渣 R₂') }} {{ slagInfo ? slagInfo.r2.toFixed(2) : '—' }}</span>
          <span class="sb-spacer"></span>
          <span class="sb-item">{{ t(ax.label) }} {{ fmt(curX) }} {{ ax.unit }}</span>
          <span class="sb-item sb-ok" v-if="applied">✓ {{ t('已应用') }}</span>
          <span class="sb-item sb-warn" v-else>● {{ t('待应用') }}</span>
          <span class="sb-sep"></span>
          <span class="sb-item mono">{{ t('Esc 关闭') }}</span>
        </div>
      </div>
    </div>
  </Teleport>
...
// try/catch 兜底：配煤成分在极端/非法状态下（如比例未归一化、成分为空）不让置换比计算抛错拖垮整个弹窗渲染
const replacementRatio = computed(() => {
  try {
    return calcReplacementRatio(compositionToFuelConfig(blendedComposition(store.materialOverrides || {})))
  } catch (e) {
    console.warn('[tft] replacementRatio 计算异常，回退默认 0.89', e)
    return 0.89
  }
})
...
const cur = computed(() => {
  try {
    ...
    return { tft: 0, status: { code: 'err', label: '异常', color: 'var(--faint)' }, co2: { CO2_emit: 0, CO2_t: 0, CO2_rate: 0, hot_metal: 0, C_in: 0, C_HM: 0, C_slag: 0, CO2_flux: 0, C_emit: 0, CO2_rate_raceway: 0, CO2_rate_other: 0, level: { code: 'err', label: '—', color: 'var(--faint)' } } }
  }
})
...
const SLAG_COLOR = { low: 'var(--accent2)', ok: 'var(--green)', high: 'var(--yellow)' }
const slagColor = computed(() => SLAG_COLOR[slagInfo.value?.level?.cls] || 'var(--faint)')
...
function curEffCoalOf(a) {
  try {
    ...
    console.warn('[tft] curEffCoalOf 计算异常，回退当前设定值', e)
    return a.key === 'coal_inj' ? curX.value : curValOf(a)
  }
}
...
const note = computed(() => {
  ...
  const a = ax.value
  const s = seriesMap.value[a.key] || []
  if (!s.length) return { kind: 'warn', text: t('扫描失败，请检查高炉工艺参数是否有效。') }
  ...
  if (a.key === 'wind_rate' && spread < 5) {
    text = t('曲线近水平：风量↑同时放大供氧与 N2 稀释，两通道抵消，TFT 对风量不敏感——风量用于调节产量，不宜作为温度/减碳调节手段。')
  } else if (a.key === 'hot_blast_temp') {
    text = t('曲线{趋势}（每 +10℃ 约 TFT {v}℃）：{desc}', { 趋势: slope > 0 ? t('线性上升') : t('下降'), v: (slope * 10).toFixed(0), desc: t('热风温度直接注入鼓风显热（分子），是最便宜、零碳排放的升温手段，为联动降焦腾出减碳空间。') })
  } else if (a.key === 'o2_flow') {
    text = t('曲线{趋势}（纯氧流量↑→富氧率派生↑）：{desc}', { 趋势: slope > 0 ? t('上升') : t('下降'), desc: t('纯氧不含 N2，注入主风管压缩 N2 分母并提升供氧，同时富氧每升高约 1% 允许多喷 15 kg/t 煤粉（富氧升温 → 燃烧带容纳更多煤粉），经置换联动降焦；煤粉热解吸热会抵消部分 TFT 增益，注意纯氧由空分供给、其电耗计入间接排放。') })
  } else if (a.key === 'coke_rate') {
    text = t('曲线{趋势}（每 +10 kg/tFe 约 TFT {v}℃）：{desc}', { 趋势: slope > 0 ? t('线性上升') : t('下降'), v: (slope * 10).toFixed(0), desc: t('焦比是风口碳的直接来源，焦比↑ → 燃烧放热↑ → TFT↑，CO₂排放同步线性↑。降焦是最直接的减碳路径，但需风温/富氧/喷煤补偿热量缺口——本轴为独立设定，仅焦比变动、煤比冻结不反推。') })
  } else if (a.key === 'coal_inj') {
    text = t('曲线{趋势}（每 +10 kg/tFe 约 TFT {v}℃）：{desc}', { 趋势: slope < 0 ? t('缓降') : t('上升'), v: (slope * 10).toFixed(0), desc: t('喷煤替代焦炭可减碳，但热解吸热与产 H2O 稀释压低 TFT，需风温/富氧补偿。') })
  }else if(a.key == 'blast_humidity'){
    text = t('曲线{趋势}（每+1g/Nm³ TFT 约下降 {v}℃）：{desc}', { 趋势: slope < 0 ? t('线性上升') : t('上升'), v: (6).toFixed(0), desc: t('鼓风湿度上升会带来高炉内部反应H2的比例上升，可以一定程度降低直接还原度，但是收益抵不过水分解的吸热损失，因此要尽量减少。') })
  } else {
    text = t('该轴全范围 TFT 变化 {v}℃。', { v: spread.toFixed(1) })
  }
  if (st.code === 'low') text += t(' 注意：当前 TFT 偏低，应先升温（风温/富氧）恢复热制度，再实施减碳。')
  if (st.code === 'high') text += t(' 注意：当前 TFT 偏高，本身即是减碳信号，可优先降焦比。')
  const c2 = cur.value.co2
  if (c2 && Number.isFinite(c2.CO2_rate)) {
    text += t(' 当前工况 CO₂ {rate} tCO₂/h（强度 {it} t/tHM，{level}）。', { rate: c2.CO2_rate.toFixed(1), it: c2.CO2_t.toFixed(3), level: t(c2.level.label) })
  }
...
const advices = computed(() => {
  ...
  // 碳排现状（配料比 → CO2 同步结论）
  if (c2 && Number.isFinite(c2.CO2_rate)) {
    list.push({ level: c2.level.code === 'high' ? 'w' : 'g', text: t('当前 CO₂ 排放 {rate} tCO₂/h（强度 {it} t/tHM，{level}）：{desc}', { rate: c2.CO2_rate.toFixed(1), it: c2.CO2_t.toFixed(3), level: t(c2.level.label), desc: t('排放随配料比联动——喷煤↑置换焦炭↓可减碳，TFT 回落到下限即该工况的碳排最优解。') }) })
  }
  if (st.code === 'low') {
    list.push({ level: 'w', text: t('当前 TFT {v}℃ 偏低：先恢复热制度再谈减碳——首选提升热风温度（免费显热），其次提高富氧率（压缩 N2），可少量降低喷煤比（减少热解吸热）。', { v: cur.value.tft.toFixed(0) }) })
  } else if (st.code === 'high') {
    list.push({ level: 'w', text: t('当前 TFT {v}℃ 偏高：本身即是减碳信号——提高喷煤比（以氢代碳、置换焦炭），必要时降低风温/富氧让 TFT 回落到合规带。', { v: cur.value.tft.toFixed(0) }) })
  } else {
    list.push({ level: 'g', text: t('当前 TFT {v}℃ 处于合规区间，可安全实施节能减碳。', { v: cur.value.tft.toFixed(0) }) })
  }
  list.push({ level: 'g', text: t('① 风温打满：热风温度提到上限，鼓风显热免费顶替焦炭放热，焦比联动下降、创造减碳空间（成本最低、零碳排放）。') })
  list.push({ level: 'g', text: t('② 用 TFT 空间换碳：提高喷煤比（置换焦炭）或提升风温/富氧联动降低焦比，TFT 随之回落，扣到下限即减碳极限。') })
  list.push({ level: 'g', text: t('③ 富氧兜底：喷煤受限时，提高富氧率压缩 N2 分母，释放新一轮提煤/降焦空间。') })
  list.push({ level: 'g', text: t('④ 监控闭环：全程盯 TFT 状态，达到下限即停手——TFT 下限就是该工况的碳排最优解。') })
  return list
})
...
<style scoped>
/* ============ 平台统一风格（跟随全局浅色/深色主题，CSS 变量体系） ============
   弹窗整体对齐平台弹窗规范（与 SystemSettingsDialog 等一致）：
   —— 标题栏 var(--bar) + border-bottom var(--border)
   —— 主体 var(--panel-2) 底 + 内容卡片 var(--panel) 浮起
   —— 品牌黄 accent（按钮 hover / 选中 / 滑块圆头 / 关键数值）
   —— 底部状态栏 var(--bar) + border-top var(--border)
   —— 控件（按钮/滑块/表格/滚动条）全部使用全局 CSS 变量，随平台浅色/深色主题自动切换 */
.tad-mask {
...
.tad-dialog {
  ...
  box-shadow: 0 18px 50px rgba(0, 0, 0, 0.28), 0 4px 14px rgba(0, 0, 0, 0.12);
  color: var(--text);
}
.tad-dialog.dragging {
  box-shadow: 0 18px 50px rgba(0, 0, 0, 0.28), 0 0 0 1px var(--accent);
  cursor: grabbing;
}

/* ---- 标题栏（平台弹窗标题栏：bar 底 + border 分隔） ---- */
.tad-titlebar {
  display: flex; align-items: center; gap: 9px; flex: none;
  background: var(--bar); padding: 0 8px 0 13px; height: 36px;
  border-bottom: 1px solid var(--border);
  cursor: move; user-select: none;
}
.tad-titlebar:active { cursor: grabbing; }
.tad-icon { color: var(--accent-d); font-size: 13px; }
.tad-title { font-size: 12.5px; font-weight: 600; color: var(--text); letter-spacing: 0.3px; }
.tad-subtitle {
  font-size: 11px; font-weight: 400; color: var(--muted); letter-spacing: 0.2px;
  margin-left: 2px;
}
.st-dot { width: 7px; height: 7px; border-radius: 50%; display: inline-block; }
.tad-spacer { flex: 1; }
.tad-restore {
  height: 26px; padding: 0 12px; border: 1px solid var(--border); background: var(--panel-2);
  color: var(--muted); border-radius: 4px; font-size: 11.5px; cursor: pointer;
  display: inline-flex; align-items: center; letter-spacing: 0.3px;
  transition: color 0.12s, border-color 0.12s, background 0.12s;
}
.tad-restore:hover:not(:disabled) { color: var(--accent-d); border-color: var(--accent-d); background: var(--accent-l); }
...
/* ---- 主体：左图表区 + 右信息区（panel-2 底 + 内容卡片 panel 浮起） ---- */
...
.tad-cell {
  position: relative; background: var(--panel); border: 1px solid var(--border); border-radius: 4px;
  padding: 4px 4px 2px; cursor: pointer;
  transition: border-color 0.12s, box-shadow 0.12s;
}
.tad-cell:hover { border-color: var(--accent-d); }
.tad-cell.on { border-color: var(--accent); box-shadow: inset 0 0 0 1px var(--accent); }
.cell-head {
  display: flex; justify-content: space-between; align-items: baseline; gap: 6px;
  padding: 0 4px 3px; font-size: 11px;
}
.cell-name { color: var(--muted); font-weight: 600; white-space: nowrap; }
.tad-cell.on .cell-name { color: var(--accent-d); }
.cell-val { color: var(--text); font-size: 11px; }
.cell-eff { font-size: 9px; color: var(--faint); margin-left: 2px; }
...
.tad-tip {
  ...
  background: var(--bar); color: var(--text); padding: 5px 9px; border-radius: 4px;
...
.tad-tip b { color: var(--accent-d); }

/* ---- 参数设定滑块（平台输入控件风格） ---- */
.tad-setter {
  flex: none; background: var(--panel); border: 1px solid var(--border); border-radius: 4px;
  padding: 8px 11px 6px; display: flex; flex-direction: column; gap: 1px;
}
.ts-head { display: flex; align-items: center; gap: 9px; }
.ts-lbl { font-size: 12px; font-weight: 600; color: var(--accent-d); }
...
.ts-badge.ok { background: rgba(46, 139, 87, .12); color: var(--green); }
.ts-badge.ok .bd-dot { background: var(--green); }
.ts-badge.dirty { background: rgba(168, 129, 26, .12); color: var(--yellow); }
.ts-badge.dirty .bd-dot { background: var(--yellow); }

/* 滑块：继承平台全局 input[type=range]（3px 滑轨 + 品牌黄圆头） */
...
.tad-note.ok { background: rgba(46, 139, 87, .12); border-color: var(--green); color: var(--green); }
...
.co2-tip {
  ...
  background: rgba(46, 139, 87, .08); border: 1px solid var(--border); border-radius: 2px; padding: 4px 7px;
}
...
.slag-val { font-size: 20px; font-weight: 700; color: var(--text); line-height: 1; }
.slag-grid {
  display: grid; grid-template-columns: 1fr 1fr; gap: 3px 10px;
}
...
.slag-th { background: var(--panel-3); color: var(--faint); font-weight: 600; }
.slag-sum { background: var(--panel-2); color: var(--text); font-weight: 600; }
.slag-neg { color: var(--red); }
...
.tad-sec-title .sec-sub {
  font-size: 10px; color: var(--faint); font-weight: 400; letter-spacing: 0.2px;
...
.d-k { color: var(--faint); white-space: nowrap; width: 56px; flex: none; }
.d-v { color: var(--text); flex: 1; white-space: nowrap; }
.d-delta { font-weight: 600; white-space: nowrap; }
.d-delta.good { color: var(--green); }
.d-delta.bad { color: var(--red); }
.d-delta.flat { color: var(--faint); }
.d-foot {
...
.tad-table tr.on .c-name { color: var(--accent-d); }
.tad-table .c-name { font-weight: 600; color: var(--accent-d); }
...
.tad-table .trend.down { color: var(--accent-d); }
...
.tad-advice .lv-g { background: rgba(46, 139, 87, .12); border-color: var(--green); color: var(--green); }
...
/* ---- 底部状态栏（平台状态栏：bar 底 + border 分隔 + 语义色） ---- */
.tad-statusbar {
  flex: none; display: flex; align-items: center; gap: 9px;
  height: 24px; padding: 0 10px;
  background: var(--bar); color: var(--muted); font-size: 11px;
  border-top: 1px solid var(--border);
  user-select: none; letter-spacing: 0.2px;
}
.tad-statusbar .sb-left { font-weight: 600; color: var(--text); }
.tad-statusbar .sb-item { display: inline-flex; align-items: center; gap: 5px; white-space: nowrap; }
.tad-statusbar .sb-dot { width: 7px; height: 7px; border-radius: 50%; display: inline-block; }
.tad-statusbar .sb-sep { width: 1px; height: 11px; background: var(--border); }
.tad-statusbar .sb-spacer { flex: 1; }
.tad-statusbar .sb-ok { font-weight: 600; color: var(--green); }
.tad-statusbar .sb-warn { font-weight: 600; color: var(--yellow); }
.tad-statusbar .sb-item.mono { font-family: var(--mono); opacity: .85; }
/* 滚动条：继承平台全局细滚动条样式 */
