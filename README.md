...
- **碳素流守恒审计**：全流程碳素流守恒审计（碳输入 = 排 CO₂ + 固钢 + 入渣 + 捕集 + 产品携出）；
- **AI 优化模型**：内置序列预测算法（SEQ，预测未来工况，属性面板可选 LSTM / LightGBM / XGBoost、设定预测目标与影响变量，时间序列大模型暂不实现）/ 强化学习优化策略（RL）/ 遗传算法优化策略（GA，适用于设备启停与连续参数复合场景）/ 粒子群优化策略（PSO，适用于连续参数最优解探索）/ 聚类工况识别（CLU，K-Means / DBSCAN / 层次聚类自动划分低中高负荷典型工况）五套在线自学习模型，按类别（时序预测 / 参数优化 / 聚类分析）列出模型列表，关闭某个模型面板时自动退回模型列表；策略模型支持设定决策变量（参与优化的工艺参数）与优化目标（碳强度 / 能耗 / 排放），聚类模型支持设定聚类算法与聚类特征变量，随实时数据后台持续训练，输出最优设备参数建议或工况分布，支持自动化控制、版本管理与调优提醒；
...
- **能碳一体机管理**（视图 → 能碳一体机管理）：集成参考项目 yunduan1 全部能力，六页签——总览（云端 Broker $SYS 实时统计 + 边缘节点 + CloudCore/证书演示）、设备管理（KubeEdge DeviceModel/Device 五协议 CRUD + YAML 生成，`config/box_devices.json`）、设备关联（云端设备 ↔ 仿真设备实例，`config/links.json`）、盒子一键接入（三种方式：① GitHub 托管——平台同步接入资产到用户仓库，盒子现场 `curl … | bash` 一条命令接入；② 自解压 `onboard_box.sh`——内嵌 box-deploy 采集包 + edgecore.yaml + rootCA + token，下载后现场执行；③ 云端 agent 远程一键接入推送执行；另仓库 `deploy/` 脚本支持全新服务器（`deploy/server.sh`）与全新盒子（`deploy/box.sh`）直接从公开源码仓库一键部署，免平台预同步；`config/edgecore.template.yaml`）、实时数据（Device.twins 真实读数 + 趋势 + 消息流 + 发测试消息）、接入指引（盒子采集 mapper / 部署包 / 诊断工具）；总体架构：云端（172.19.134.45）K3s 轻量 K8s + KubeEdge CloudCore 作为唯一控制平面；盒子边缘仅安装 EdgeCore（不部署 k3s-server，无本地控制平面），经 CloudHub 10002 端口长连接云端，边缘运行 Pod / mosquitto / box-mapper；
...
│   │   ├── stores/             # Pinia 全局状态（sim.js 流程/仿真/实验；audit.js 审计）
